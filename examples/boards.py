...
def board_cover(board, pieces, rotations, optional=None):
    ...
    if optional == 'board':
        optional = list(range(len(pieces), len(cols)))
    elif optional == 'pieces':
        optional = list(range(len(pieces)))
    else:
        optional = []
    return (pairs, optional, rows)
...
if __name__ == '__main__':
    ...
    rotations = {tuple(tuple(row) for row in np.array(a).dot(b).dot(c).tolist())
                 for a, b, c in product(*(matrix_powers(r, 4)
                                          for r in [x, y, z]))}
    ...
    with open('battleship.txt', 'w') as f:
        print_cover(board_cover(board, pieces, rotations, 'board'), f)
    ...
    def hex_battleship(is_upper):
        ...
        return board_cover(board, pieces, rotations, 'board')
